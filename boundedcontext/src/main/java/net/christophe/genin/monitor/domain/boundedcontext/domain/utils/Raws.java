...
public final class Raws {
    ...
    public static List<String> getTablesFrom(JsonObject json) {
        return Jsons.builder(json.getJsonArray(Schemas.Raw.Tables.collection())).toStream()
                .map(js -> js.getString(Schemas.Raw.Tables.table.name(), ""))
                .collect(Collectors.toList());
    }
    ...
    public static JsonObject getApisFrom(JsonObject json) {
        return json.getJsonObject(Schemas.Raw.apis.name(), new JsonObject());
    }

    public static String getVersionFrom(JsonObject json) {
        return json.getString(Schemas.Raw.Apis.version.name(), "");
    }

    public static String getArtifactIdFrom(JsonObject json) {
        return json.getString(Schemas.Raw.Apis.artifactId.name(), "");
    }
