...
public class ConfigurationCommand extends AbstractVerticle {
    ...
    public void start() {
        vertx.eventBus().consumer(SAVE, (Handler<Message<JsonObject>>) (msg) -> {
            ...
            ConfigurationDomain configurationDomain = new ConfigurationDomain(iConfiguration);
            configurationDomain.load()
                    .map(setDatas(body))
                    .flatMap(configurationDomain::save)
                    .subscribe(bool -> {
    ...
    Func1<Configuration, Configuration> setDatas(JsonObject body) {
        return configuration -> {
            ...
            configuration.setConfId(body.getLong("id", 1L))
                    ...
                    .setMonithorUrl(body.getString("moniThorUrl"))
