...
public class Adapters extends AbstractVerticle {
    ...
    public IConfiguration configurationHandler(){
        return new NitriteConfiguration.ConfigurationHandler(NitriteDbs.instance);
    }

    public IDependency dependencyHandler() {
        switch (type.getType()) {
            case MYSQL:
                return new MysqlDependency.MysqlIDependency(Mysqls.Instance.get());
            case NITRITE:
                return new NitriteDependency.NitriteIDependency(NitriteDbs.instance);
            default:
                throw new IllegalStateException("No type found " + type);
        }
    }

    public IRaw rawHandler() {
        switch (type.getType()) {
            case MYSQL:
                return new MysqlRaw.MysqlIRaw(Mysqls.Instance.get());
            case NITRITE:
            default:
                return new NitriteRaw.NitriteIRaw(NitriteDbs.instance);
        }
    }
    ...
    public IStoredServiceEvent storedServiceEventHandler() {
        switch (type.getType()) {
            case MYSQL:
                return new MysqlStoredServiceEvent.MysqlIStoredServiceEvent(Mysqls.Instance.get());
            case NITRITE:
            default:
    ...
    public IFrontApps frontAppsHandler() {
        switch (type.getType()) {
            case MYSQL:
                return new MysqlFrontApps.MysqlIFrontApps(Mysqls.Instance.get());
            case NITRITE:
                return new NitriteFrontApps.NitriteIFrontApps(NitriteDbs.instance);
            default:
                throw new IllegalStateException("Only in mysql mode " + type);
        }
    }
