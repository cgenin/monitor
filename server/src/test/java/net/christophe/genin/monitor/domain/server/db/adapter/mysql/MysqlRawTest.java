...
public class MysqlRawTest extends DbWithSchemaTest implements ReadJsonFiles {
    ...
    public void should_update_state(TestContext context) {
        ...
        rawDomain.save(data)
                .flatMap(raw -> rawDomain.updateAllStatesBy(Treatments.TABLES))
                .subscribe(nb -> {
                    context.assertTrue(nb > 0);
                    rawDomain.findByStateFirst(Treatments.TABLES)
                            .subscribe(raw -> {
    ...
    private void should_find_all(Async async, TestContext context, long l) {
        ...
        rawDomain.findAll().subscribe(raw -> {
    ...
    private void should_find_by_state(Async async, TestContext context, long l) {
        ...
        rawDomain.findByStateFirst(Treatments.PROJECTS)
                .subscribe(raw -> {
        ...
        rawDomain.findByStateFirst(Treatments.VERSION)
                .subscribe(raw -> {
